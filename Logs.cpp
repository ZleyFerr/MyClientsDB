...
void printLog(std::string LogMessage)
{
  ...
  if (localtime_s(&tstruct, &now) != 0)
  {
    currentLogFile << "[ERROR] Ошибка получения времени.\n";
    std::cerr << "Ошибка получения времени." << std::endl;
    return;
  }

  std::stringstream ss;
  ss << std::put_time(&tstruct, "%H:%M:%S");
...
void printLog()
{
  ...
  ss << std::put_time(&tstruct, "%H:%M:%S");
  std::string formattedTime = ss.str();
  currentLogFile <<  "[ANY] [" << formattedTime << "] ";
}
